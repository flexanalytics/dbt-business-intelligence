...
{%- macro sqlserver__create_not_null(table_relation, column_names, verify_permissions, quote_columns=false) -%}
    ...
    {%- if dbt_constraints.have_ownership_priv(table_relation, verify_permissions) -%}
            ...
            {%- for column in columns_list -%}
                ...
                {%- do log("Creating not null constraint for: " ~ column ~ " in " ~ table_relation, info=true) -%}
                {%- call statement('add_nn', fetch_result=False, auto_begin=True) -%}
                    ALTER TABLE {{table_relation}} ALTER COLUMN {{column}} {{col_type}} NOT NULL;
                {%- endcall -%}
...
{%- macro sqlserver__unique_constraint_exists(table_relation, column_names) -%}
    ...
    WHERE tc.constraint_type in ('PRIMARY KEY', 'UNIQUE')
        AND upper(kc.table_schema) = upper('{{table_relation.schema}}')
        AND upper(kc.table_name) = upper('{{table_relation.identifier}}')
    ...
    {%- if constraint_list.columns["column_name"].values() | count > 0 -%}
        {%- for constraint in constraint_list.group_by("constraint_name") -%}
            {%- if dbt_constraints.column_list_matches(constraint.columns["column_name"].values(), column_names ) -%}
                ...
                {{ return(true) }}
                {%- do log("DID NOT FIND PK/UK key: " ~ table_relation ~ " " ~ column_names, info=false) -%}
            {%- endif -%}
        {% endfor %}
    {%- endif -%}#}
...
{% macro sqlserver__drop_referential_constraints(relation) -%}
    {%- do log("sqlserver__drop_referential_constraints from table " ~ relation, info=true) -%}

    {%- set fk_lookup_query -%}
    EXEC sp_fkeys '{{relation.identifier}}'
    {%- endset -%}

    {%- for row in run_query(fk_lookup_query) -%}
        {%- do log("Dropping foreign key: " ~ row.values()[11], info=true) -%}
        {%- call statement('drop_fk', fetch_result=False, auto_begin=True) -%}
        ALTER TABLE {{ row.values()[6] }} DROP CONSTRAINT IF EXISTS "{{ row.values()[11] }}"
        {%- endcall -%}
        {{ adapter.commit() }}
        {%- do log("Foreign key dropped: " ~ row.values()[11], info=true) -%}
    {% endfor %}
